...
class CJsTree extends CWidget
{
	...
	public $callback;
        public $type;
        public $plugins = array('themes', 'json');
    ...
    public function init()
    {
        ...
        $cs->registerScriptFile($this->baseUrl.'/jstree.core.js');
        $cs->registerScriptFile($this->baseUrl.'/vakata.js');
        foreach ($this->plugins as $plugin) {
        	if ($plugin === 'json') $plugin='json';
        	$cs->registerScriptFile($this->baseUrl."/jstree.$plugin.js");
        }
        ...
        if (!isset($this->options['themes'])) $this->options['themes'] = array();
        ...
        $options = preg_replace("/'#DATA#'/Us", "function (n,o) {return { 'id' : n!=-1 ? $(n).data('id') : 0 };} \n", $options);
	...
	{ 
		...
		//$this->body=$this->body.self::saveDataAsHtml($this->data);
                //$this->body=$this->body.self::saveDataAsJson($this->json);
		$this->body=$this->body."\n</ul>";
	...
	protected function getClientOptions()
	{
		$options=$this->options;
                ...
                $optionsParams=array('selected','opened','languages','path','cookies','ui','rules','lang','callback','plugins');
                
                if (!isset($this->json[0]) )
                {
